...
import { assessExtractionQuality, normalizeReadableText, type StoredWritingProfile, type WritingProfileMetrics } from "../_shared/text-analysis.ts";
import type { InternalSimilaritySubmission } from "../_shared/integrity-provider-runners.ts";

export type IntegrityType = "similarity" | "ai-writing" | "baseline-deviation" | "mixed";
...
export function toProviderSubmission(submission: SubmissionRow): InternalSimilaritySubmission {
  return {
    id: submission.id,
...
export type EvidenceItem = { label: string; value: string; score: number };

export type ProcessedSubmissionText = {
  originalText: string;
  mainBody: string;
...
export function preprocessSubmissionText(text: string): ProcessedSubmissionText {
...
export function buildFallbackEvidenceItem(label: string, value: string, score: number): EvidenceItem {
...
export function normalizeArtifactDrivenReason(params: {
  reason: string;
...
export function classifySimilarityContext(
  excerpt: string,
...
export function deriveCitationAwareOverlap(params: {
...
export function normalizeScoresByContext(
  similarityScore: number,
  aiSuspicionScore: number,
  severity: "low" | "medium" | "high",
  integrityType: IntegrityType,
  recommendedAction: "clear" | "review" | "investigate",
) {
...
export function computeRisk(similarity: number, aiSuspicion: number, baselineDeviation: number) {
  return Math.round(similarity * 0.4 + aiSuspicion * 0.3 + baselineDeviation * 0.3);
}

export function severityFromRisk(score: number): "low" | "medium" | "high" {
  if (score >= 80) return "high";
  if (score >= 55) return "medium";
  return "low";
}

export function actionFromRisk(score: number): "clear" | "review" | "investigate" {
  if (score >= 80) return "investigate";
  if (score >= 45) return "review";
  return "clear";
}

export function crossesIntegrityThreshold(snapshot: {
  totalScore: number;
  ...
  return (
    snapshot.similarityScore >= 25 ||
    snapshot.aiWritingScore >= 25 ||
    snapshot.baselineDeviationScore >= 25 ||
    snapshot.totalScore >= 25
  );
}
...
export function isRecoverablePersistenceError(error: unknown) {
...
export function resolveMossRunnerConfigFromAnalysis(config: MossRunnerConfig | null) {
  return config;
}
