...
function summarizeExtractionObservability(params: {
...
function mergeIntegrityFlags(flags: IntegrityFlag[]) {
...
export function createCheckPlagiarismHandler(deps: CheckPlagiarismHandlerDeps) {
  const requestIntegrityResponse = deps.createIntegrityResponseWithRetry ?? createIntegrityResponseWithRetry;
...
Only flag real concerns. Return valid JSON only.`,
    ...
    if (shouldRunLegacy) {
      try {
        const aiData = await requestIntegrityResponse({
    ...
    if (shouldSkipInternalSimilarityForCohortSize) {
      ...
      warnings.push(
        `Internal cohort similarity scanning was skipped because this assignment has ${comparisonSubmissions.length} submissions, exceeding the current safety limit of ${MAX_INTERNAL_COMPARISON_SUBMISSIONS}.`,
      );
    }
    ...
    if (
      ...
      const comparableSubmissions = comparisonSubmissions
        .map((submission) => {
          const content = contentMap.get(submission.id);
    ...
    if (shouldRunMossProvider && requestedAssignmentId && comparisonSubmissions.length >= 2 && mossRunnerConfig) {
    ...
    const providerFindings = [...internalFindings, ...mossFindings];
    const internalFlags = normalizeFlags(buildInternalSimilarityFlagCandidates({
      ...
      submissions: comparisonSubmissions,
    }), comparisonSubmissions, processedContentMap);
