...
export type ModerationQueueSectionProps = {
...
export const ModerationQueueHeader = ({
  queueFilter,
  queueFilterOptions,
  queueSearch,
  queueSort,
...
export const ModerationQueueFocusBanner = ({
  assignmentFocusTitle,
  onClearAssignmentFocus,
}: Pick<ModerationQueueSectionProps, "assignmentFocusTitle" | "onClearAssignmentFocus">) =>
  assignmentFocusTitle ? (
...
export const ModerationQueueBulkAssignPanel = ({
  ...
  bulkAssignableCaseIds.length > 0 ? (
    <div className="grid gap-3 rounded-xl border bg-muted/20 p-4 lg:grid-cols-[auto_minmax(0,1fr)_220px_auto] lg:items-center">
      <div className="flex items-center gap-3">
        <Checkbox
          checked={bulkAssignableCaseIds.length > 0 && bulkAssignableCaseIds.every((caseId) => selectedCaseIds.includes(caseId))}
...
export const ModerationQueueBulkApprovePanel = ({
  ...
  ModerationQueueSectionProps,
  "bulkApprovableCaseIds" | "onBulkApproveModeration" | "selectedBulkApprovalSummaries" | "saving"
>) =>
...
export const ModerationQueueCaseList = ({
  cases,
  onSelectCase,
...
>) =>
  cases.length === 0 ? (
    ...
    cases.map((item) => {
      ...
      return (
        <div key={item.moderationCase.id} data-testid={`moderation-case-${item.moderationCase.id}`} className="rounded-xl border p-4">
            ...
            <div className="flex gap-3">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {item.assignment?.title || "Assignment"} - Submitted {safeFormatDate(item.submission?.submitted_at, "MMM d, yyyy HH:mm")}
                </p>
                <p className="text-xs text-muted-foreground">
                  First marker: {item.firstMarker?.full_name || "Unassigned"} - Moderator: {item.moderator?.full_name || "Unassigned"}
                </p>
                {item.moderationCase.trigger_summary && <p className="text-xs text-muted-foreground">{item.moderationCase.trigger_summary}</p>}
                {item.moderationCase.status === "moderated" && <p className="text-xs text-muted-foreground">{disagreement.label}</p>}
