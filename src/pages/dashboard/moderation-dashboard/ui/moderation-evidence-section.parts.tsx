...
export const formatJsonLabel = (value: unknown) =>
  typeof value === "string" && value.trim().length > 0
    ? value.replace(/[_-]+/g, " ").replace(/\b\w/g, (letter) => letter.toUpperCase())
    : "Unavailable";

export const toAssignmentDetailSubmission = (submission: NonNullable<ModerationCaseView["submission"]>): AssignmentDetailSubmission => ({
...
export const asEvidenceList = (value: unknown) => (Array.isArray(value) ? value : []);

export const SubmissionFileCard = ({
  hasSubmissionFile,
  onOpenSubmissionFile,
  selectedCase,
}: {
  hasSubmissionFile: boolean;
  onOpenSubmissionFile: () => void;
...
export const AssignmentContextCard = ({ selectedCase }: { selectedCase: ModerationCaseView }) => (
  <div className="rounded-lg border bg-muted/20 p-3 text-sm">
...
export const RubricSection = ({ rubricItems }: { rubricItems: unknown[] }) =>
...
export const MarkingEvidenceCard = ({ selectedCase }: { selectedCase: ModerationCaseView }) => (
  <Card>
...
export const IntegrityContextCard = ({
  formatJsonLabel,
  integrityPayload,
  selectedCase,
}: {
  formatJsonLabel: (value: unknown) => string;
  integrityPayload: {
    latestNote?: string | null;
    history: Array<{ id: string; createdAt: string; decision: string; note: string }>;
  } | null;
  selectedCase: ModerationCaseView;
}) => (
  <Card>
...
export const AIBreakdownSection = ({ aiBreakdown }: { aiBreakdown: unknown[] }) =>
