...
import { buildActivityFeed, buildAssignmentSubmissionSummaryMap, toActivityTone } from "./dashboardData.helpers";
...
export const buildAdminDashboardData = ({
  ...
  const lecturerNameById = new Map(users.map((row) => [row.id, row.fullName || row.email || "Unknown lecturer"]));
  ...
  const moderationRows: AdminModerationRow[] = rpcModerationRows ?? moderationCases.map((row) => ({
    id: row.id,
