...
const sortByPriority = (left: CohortRecommendation, right: CohortRecommendation) => {
...
const lowCohortAverageId = (lecturerId: string, assignmentId: string) =>
  `low-cohort-average:${lecturerId}:${assignmentId}`;

const highFailureRateId = (lecturerId: string, assignmentId: string) =>
  `high-failure-rate:${lecturerId}:${assignmentId}`;

const scoreDropId = (lecturerId: string, currentAssignmentId: string, previousAssignmentId: string) =>
  `score-drop:${lecturerId}:${currentAssignmentId}:${previousAssignmentId}`;

const weakRubricId = (lecturerId: string, assignmentId: string, criterionKey: string) =>
  `weak-rubric:${lecturerId}:${assignmentId}:${criterionKey}`;

const highRiskClusterId = (lecturerId: string, cohortKey: string) =>
  `high-risk-cluster:${lecturerId}:${cohortKey}`;

const integritySpikeId = (lecturerId: string, assignmentId: string) =>
  `integrity-spike:${lecturerId}:${assignmentId}`;

const positiveSignalId = (lecturerId: string, cohortKey: string) =>
